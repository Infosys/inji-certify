...
> **Note (local testing only):** The `vp_request_config.json` shipped in the repository also contains
> `clientId`, `nonce`, and `responseCodeValidationRequired` fields. These are **only for local testing**
> convenience and are **ignored at runtime**. In production, the verifier `clientId` comes from the
> `mosip.certify.verify.service.verifier-client-id` property, the `nonce` is dynamically generated by
> the Inji Verify library, and `responseCodeValidationRequired` is always `false`.
