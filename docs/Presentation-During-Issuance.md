...
Certify interacts with the VP Verifier to create a presentation request using DCQL (Digital Credentials Query Language), which is then sent back to the Wallet. The Wallet retrieves the presentation request and prompts the User for consent to share existing credentials.
...
## Query Format: DCQL (Digital Credentials Query Language)

Certify uses **DCQL** (Digital Credentials Query Language) as defined in [OpenID for Verifiable Presentations 1.0 — Section 6](https://openid.net/specs/openid-4-verifiable-presentations-1_0.html#section-6) for specifying credential requirements in the VP request.

### VP Request Configuration (`vp_request_config.json`)

The VP request configuration contains a `dcqlQuery` block that specifies which credentials are requested.
...
sequenceDiagram
    ...
    IC-->>W: 9. 200 Interactive Authorization Response<br/>{status:"require_interaction", type:"urn:openid:dcp:iae:openid4vp_presentation", auth_session:"..random string", openid4vp_request: {dcql_query: {...}, response_mode: "iae_post" or "iae_post.jwt"}}
    
    Note over W,IC: 2. Presentation Flow with Issuer and VP Verifier
    W->>W: 10. Display and select credential(s) which satisfies DCQL query criteria
    W->>U: 11. User consent
    U-->>W: 12. Approve
    W->>IC: 13. POST Content-Type: application/x-www-form-urlencoded /iae<br/>{auth_session=...&openid4vp_response={"vp_token": ...}}
    IC->>IC: 14. Validate auth_session
    IC->>IVP: 15. Submit vp_token + state to verify VP
...
2. **Inji Certify**: Evaluates incoming request and identifies whether a Verifiable Presentation (VP) is required for the credential issuance.
    - If a VP is required, it proceeds to create a presentation request with the VP Verifier using DCQL.
    - If no VP is required, it continues with authorization code flow. (That is as per standard OpenId4VCI spec, not included here)
3. **Inji Certify to VP Verifier**: Passes the configured `dcqlQuery` to the Verify library's `createAuthorizationRequest`.
4. **VP Verifier to Inji Certify**: Returns `request_id`, `transaction_id`, and `authorizationDetails` (e.g. `{response_type: "vp_token", response_mode: "direct_post", dcqlQuery: {...}}`). Inji Certify stores `transaction_id` mapped to the Auth Session.
5. **Inji Certify to Wallet**: Responds with `200 Interactive Authorization Response`. Includes `status:"require_interaction"`, `type:"urn:openid:dcp:iae:openid4vp_presentation"`, `auth_session`, `openid4vp_request:{dcql_query: {...}, response_mode: "iae_post" or "iae_post.jwt"}`
    - The `openid4vp_request` contains `dcql_query` (not `presentation_definition`), with `response_mode` set to either `iae_post` for unencrypted response or `iae_post.jwt` for encrypted response. [Refer](https://openid.github.io/OpenID4VCI/openid-4-verifiable-credential-issuance-1_1-wg-draft.html#name-interactive-authorization)

### Phase 2: Presentation Flow with Issuer and VP Verifier

The Wallet interacts with the VP Verifier
1. **Wallet**: Display and select credential(s) which satisfies the DCQL query criteria.
...
4. **Wallet to Inji Certify**: Send VP response
    ...
    - The `openid4vp_response` contains `vp_token` only (no `presentation_submission` in DCQL mode).
    - The `vp_token` is a JSON object keyed by credential query id (e.g., `{"mosip_verifiable_credential_id": ...}`).
    - if response_mode is `iae_post.jwt` then openid4vp_response is encrypted, {response='...'}
5. **Inji Certify**: Validates `auth_session`
6. **Inji Certify to VP Verifier**: Submits `vp_token` + `state` to the VP Verifier for verification.
...
* [OpenID for Verifiable Presentations 1.0 — DCQL (Section 6)](https://openid.net/specs/openid-4-verifiable-presentations-1_0.html#section-6)
